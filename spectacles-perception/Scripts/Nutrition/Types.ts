 ...
 * `EatingEventPayload`/`FoodAnalysisResult`, and `A5_EatingTrigger/FoodAnalysisClient.ts`'s
 * `IFoodAnalysisClient` — `GeminiFoodAnalysisClient` is what implements that contract's
 * backend side, entirely on-device.
 */
...
/** A region resolved down to its top candidate — portion estimation only cares about this. */
...
/** One food item as tracked within an eating session — the unit dedup runs against. */
