...
/**
 * Hand/object intersection helpers.
 *
 * Pure functions, no scene dependency, so both the food-in-hand classifier
 * and any debug HUD can reuse them.
 *
...
export function findNearestFoodObject(
  ...
  if (camera) {
    const nearby = foodObjects.filter((o) => isHandNearObjectScreenSpace(hand, o, camera));
    if (nearby.length > 0) {
