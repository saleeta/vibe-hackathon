...
export class WorkoutManager extends BaseScriptComponent {
  ...
  private estimateKcal(): number {
    // Rough per-rep/per-step heuristic constants, not a calibrated
    // measurement — same "estimate" framing as the nutrition side's
    // glycemic load (see docs/COMPLIANCE.md). Good enough for an MVP
    // progress number, not a fitness-tracker-grade calorie count.
    //
    ...
    // kcal-per-kg-per-km walking approximation — this is why both heightCm
    // and bodyWeightKg matter here, unlike squats/curls which only need weight.
    const strideM = (this.heightCm / 100) * 0.414;
    const stepDistanceKm = (this.steps * strideM) / 1000;
    const stepKcal = stepDistanceKm * this.bodyWeightKg * 0.9;
    const squatKcal = this.squats * (0.005 * this.bodyWeightKg);
    const curlKcal = this.curls * (this.curlWeightKg * 0.01 + 0.1);
    return stepKcal + squatKcal + curlKcal;
  }
