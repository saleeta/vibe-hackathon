 ...
 *
 * Person A calls addObservation() once per detected food item per frame —
 * either one at a time (sequential bites) or several at once via
...
export class EatingSessionManager {
  ...
  /**
   * A single frame that shows several foods at once (a plate) — e.g. B1
   * returning rice + chicken + broccoli + sauce from one image. Processes
