 ...
 * vision backend the team lands on (hosted model, an OpenRouter-hosted VLM,
 * or a custom endpoint). The interface only needs a base64 still frame in and
...
export interface FoodClassifierBackend {
   ...
   * `FoodDetection/FoodInHandClassifier.ts`) — a hint to disambiguate with,
   * never trusted blindly over what the backend itself sees in the image.
   */
 ...
 * fetch. `api/`'s current backend (OpenRouterVisionClassifier) calls
 * OpenRouter's chat completions API directly instead and implements
 * FoodClassifierBackend itself, since the model's response needs reshaping
 * into that format — this class is here for a future backend that already
 * speaks it natively.
 */
