/**
 * Lens-side port of nutrition-service's food database + lookup (B3),
 * so a Lens-only pipeline (Capture/GeminiFoodAnalysisClient.ts) can run
 * end-to-end with zero network dependency beyond the Gemini vision call
 * itself (which RemoteServiceGateway handles without a self-hosted
 * server/tunnel). Kept in sync by hand with
 * `../../../../nutrition-service/src/db/foods.json` — same placeholder
 * caveat applies: a small hand-seeded table, not a licensed nutrition
 * data source. `nutrition-service` itself is unchanged and still backs
 * `api/`'s HTTP path (test-images/, the OpenRouter-based flow) — this is
 * a second copy for the fully-local Lens path, not a replacement.
 */
